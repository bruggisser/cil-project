...
class ModelVote:
    """
    Take the majority for each prediction. To break ties, the models are weighted (see config.json).
    """
    def __init__(self, config, logger, data_loader):
        ...
        with open(f"{self.config.get('data_dir')}unified_validation_set.csv", "r") as csv_file:
            ...
            for row in reader:
                values = [(weights[column] if float(row.get(column)) >= 0.5 else 0) for column in self.selected_models]
        ...
        logger.log_metric("vote_aggregation_accuracy", (correct / (correct + wrong)))
